...
export async function POST(req: NextRequest) {
  try {
    ...
    if (templateId === "birthday") {
      ...
      const publicDir = path.join(process.cwd(), "public");
      const eventDir = path.join(publicDir, "event");

      const beigeBgBase64 = fs.readFileSync(
        path.join(eventDir, "beige-backgorund.png"),
        "base64",
      );
      const logo2Base64 = fs.readFileSync(
        path.join(eventDir, "imenalogo2.png"),
        "base64",
      );

      html = getEventTemplate({
        ...data,
        beigeBg: `data:image/png;base64,${beigeBgBase64}`,
        imenaLogo: `data:image/png;base64,${logo2Base64}`,
      });
    } else {
      html = getBirthdayTemplate(data); // Fallback
    }
...
function getBirthdayTemplate(data: {
  ...
  return `
    ...
    <head>
      ...
      <style>
        body { margin: 0; padding: 0; background-color: #45291b; font-family: 'Playfair Display', serif; color: white; width: 794px; height: 1123px; display: flex; flex-direction: column; align-items: center; overflow: hidden; position: relative; }
        .banner-container { position: absolute; top: -20px; left: -5%; width: 110%; display: flex; justify-content: center; gap: 2px; z-index: 1; }
        .flag { width: 70px; height: 100px; clip-path: polygon(0 0, 100% 0, 100% 100%, 50% 85%, 0 100%); margin: 0 2px; }
        .flag:nth-child(1) { background-color: #f1c98e; }
        .header { margin-top: 130px; text-align: center; z-index: 10; }
        .header .happy { font-size: 36px; letter-spacing: 0.4em; text-transform: uppercase; margin-bottom: -15px; }
        .header .birthday { font-family: 'Great Vibes', cursive; font-size: 110px; margin: 0; line-height: 1; }
        .header .recipient-name { font-family: 'Great Vibes', cursive; font-size: 60px; color: white; margin-top: -15px; }
        .photo-frame { margin-top: 25px; background: white; padding: 18px; padding-bottom: 70px; transform: rotate(-1deg); width: 440px; z-index: 5; }
        .photo-frame img { width: 100%; height: 480px; object-fit: cover; }
        .personal-message { margin-top: 40px; width: 80%; text-align: center; font-size: 22px; font-style: italic; }
      </style>
    </head>
    <body>
       <div class="header">
        ...
        <div class="photo-frame"><img src="${image}" alt="Moment"></div>
...
function getKwibukaTemplate(data: {
  ...
  date: string;
  venue: string;
  messageOfHope: string;
  imenaLogo: string;
  kwibukaIcon: string;
  kwibukaBg: string;
}) {
  const { years, date, venue, kwibukaIcon, kwibukaBg } = data;
  return `
    <!DOCTYPE html>
    <html>
    <head>
        ...
        <div class="date-container">
          ${date
            ...
            .map(
              (part, i, arr) => `
            <span>${part.trim()}</span>
            ${i < arr.length - 1 ? '<span class="date-separator">/</span>' : ""}
          `,
            )
            .join("")}
        </div>
...
function getEventTemplate(data: {
  eventDate: string;
  eventDay: string;
  hostingFamily: string;
  location: string;
  imenaLogo: string;
  beigeBg: string;
}) {
  const { eventDate, eventDay, hostingFamily, location, imenaLogo, beigeBg } =
    data;

  // Parse eventDay field (e.g., "7 May 2025" or "12th January 2026")
  // Extract day number, month name, and year
  const dateParts = eventDay.trim().split(/\s+/);

  // Handle formats like "7 May 2025" or "7th May 2025"
  const dayRaw = dateParts[0] || "1";
  const day = dayRaw.replace(/\D/g, "") || "1"; // Remove non-digits (like "th", "st", "nd", "rd")
  ...
  return `
    ...
    <body>
      <div class="card-arch">
        ...
        <div class="hosted-by">HOSTED BY ${hostingFamily.toUpperCase()}</div>

        <div class="date-section">
          <div style="display: flex; flex-direction: column; flex: 1; align-items: center;">
            <div class="date-divider" style="width: 100%;"></div>
